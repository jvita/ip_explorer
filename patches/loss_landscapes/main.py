...
# noinspection DuplicatedCode
def random_line(model_start: typing.Union[torch.nn.Module, ModelWrapper], metric: Metric,
                n_lines=1, distance=0.1, steps=100, normalization='filter',
                deepcopy_model=False, n_loss_terms=1) -> np.ndarray:
    """
    ...
    :param num_lines: the number of lines to be evaluated
    ...
    :param deepcopy_model: indicates whether the method will deepcopy the model(s) to avoid aliasing
    :param n_loss_terms
    :return: a shape (`n_lines`) array of loss values along the randomly sampled direction
    """
    # create wrappers from deep copies to avoid aliasing if desired
    # force deepcopy to ensure each line restarts from start
    model_start_wrapper = wrap_model(copy.deepcopy(model_start) if deepcopy_model else model_start)

    # Also makes sure start_point is on correct device
    print("Verifying initial losses:", metric(model_start_wrapper))
    ...
    for i in range(n_lines):
        ...
        backstep_dir = np.floor(steps/2)  # to handle odd numbers of steps properly         
        direction.mul_(backstep_dir)
        start_point.sub_(direction)
        direction.truediv_(backstep_dir)

        tmp = []
        logging.info(f'Random line progress (outer loop): {i+1}/{n_lines}')
        for j in range(steps):
            # add a step along the line to the model parameters, then evaluate
            tmp.append(metric(model_start_wrapper))
            start_point.add_(direction)

            logging.info(f'\tRandom line progress (inner loop): {j+1}/{steps}')

        data_values.append(tmp)

        # move start point back to original start point
        reset_dir = steps - backstep_dir
        direction.mul_(reset_dir )
        start_point.sub_(direction)
        direction.truediv_(reset_dir)
...
def random_plane(model: typing.Union[torch.nn.Module, ModelWrapper],
                 metric: Metric, distance=1, steps=20,
                 normalization='filter', deepcopy_model=False, n_loss_terms=1,
                 ) -> np.ndarray:
    """
    ...
    :param distance: maximum distance in parameter space from the start point
    :param steps: at how many steps from start to end the model is evaluated
    :param steps: number of steps along dir2. Defaults to `steps`
    :param normalization: normalization of direction vectors, must be one of 'filter', 'layer', 'model'
    :param deepcopy_model: indicates whether the method will deepcopy the model(s) to avoid aliasing
    :param n_loss_terms: the number of values returned by the metric. Default=1.
    :param dir_one: the first direction along which to compute the plane. Random if not provided
    :param dir_two: the first direction along which to compute the plane. Random if not provided
    :param shift_to_center: if True, shifts the starting point to the center of the plane
    :return: 1-d array of loss values along the line connecting start and end models
    """
    ...
    # Sanity checks
    print(
        "Distance magnitudes (model, dir_one, dir_two):",
        start_point.model_norm(),
        dir_one.model_norm(),
        dir_two.model_norm(),
    )

    # Move start point so that original start params will be in the center of the plot
    backstep_dir1 = np.floor(steps/2)  # to handle odd numbers of steps properly         
    dir_one.mul_(backstep_dir1)
    start_point.sub_(dir_one)
    dir_one.truediv_(backstep_dir1)

    backstep_dir2 = np.floor(steps/2)  # to handle odd numbers of steps properly         
    dir_two.mul_(backstep_dir2)
    start_point.sub_(dir_two)
    dir_two.truediv_(backstep_dir2)

    data_matrix = [list() for _ in range(n_loss_terms)]
    # evaluate loss in grid of (steps * steps) points, where each column signifies one step
    # along dir_one and each row signifies one step along dir_two. The implementation is again
    # a little convoluted to avoid constructive operations. Fundamentally we generate the matrix
    # [[start_point + (dir_one * i) + (dir_two * j) for j in range(steps)] for i in range(steps].
    for i in range(steps):
        data_column = [list() for _ in range(n_loss_terms)]

        logging.info(f'Random plane progress (outer loop): {i+1}/{steps}')
        for j in range(steps):
            # for every other column, reverse the order in which the column is generated
            # so you can easily use in-place operations to move along dir_two
            if i % 2 == 0:
                returns = np.atleast_1d(metric(model_start_wrapper))

                for vi, v in enumerate(returns):
                    data_column[vi].append(v)
                ...
                for vi, v in enumerate(returns):
                    data_column[vi].insert(0, v)

                start_point.sub_(dir_two)

            logging.info(f'\tRandom plane progress (inner loop): {j+1}/{steps}')

        for vi in range(n_loss_terms):
            data_matrix[vi].append(data_column[vi])

        start_point.add_(dir_one)

    data_matrix = [np.array(dm) for dm in data_matrix]
    if n_loss_terms == 1: data_matrix = data_matrix[0]
