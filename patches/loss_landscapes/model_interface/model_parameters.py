    ...
    def filter_normalize_(self, ref_point: 'ModelParameters', order=2):
        ...
        for l in range(len(self.parameters)):
            ...
            if len(self.parameters[l].size()) == 1:
                norm = self.parameters[l].norm(order)
                if norm:
                    self.parameters[l] *= (ref_point.parameters[l].norm(order) / norm)
                else:
                    # To avoid divide-by-zero
                    pass
            # normalize two-dimensional weight vectors
            for f in range(len(self.parameters[l])):
                norm = self.filter_norm((l, f), order)
                if norm:
                    self.parameters[l][f] *= ref_point.filter_norm((l, f), order) / norm
                else:
                    # To avoid divide-by-zero
                    pass
...
def rand_u_like(example_vector: ModelParameters) -> ModelParameters:
    ...
    for param in example_vector:
        new_vector.append(torch.rand(
            size=param.size(),
            dtype=example_vector[0].dtype,
            device=example_vector[0].device
        ))
