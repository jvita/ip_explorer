...
class ACE:
    """
    This is an exact copy of tensorpotential/potential/ace.py, but replacing any
    TensorFlow variables/tensors/functions with their PyTorch counterparts.
    """
    def __init__(self, potconfig, nelements=1, rcut=5, lmbda=5.25, nradmax=5, lmax=4, nradbase=12,
        ...
        self.fit_coefs = torch.Parameter(self.tmp_coefs, dtype=torch.float64)
        ...
        self.factor4pi = torch.sqrt(4 * np.pi)

    def compute_regularization(self):
        basis_coefs = self.fit_coefs[self.total_num_crad:]
        self.reg_l1 = torch.sum(torch.abs(basis_coefs))
        self.reg_l2 = torch.sum(basis_coefs ** 2)
            ...
            for f in basis:
                ...
                for _ in f.coeffs:
                    if r == 1:
                        ...
                        count += 1
                rank = max([rank, r])
            self.coefs_part[ne] = torch.as_tensor(self.coefs_part[ne], dtype=torch.int32)
            self.ranksmax += [rank]
        self.coefs_r1_part = {c: torch.as_tensor(self.coefs_r1_part[c], dtype=torch.int32) for c in self.bond_combs}
    ...
    def set_coefs(self, coefs):
        self.fit_coefs.assign(torch.Parameter(coefs, dtype=torch.float64))
    ...
    def cheb_exp_cos(self, d_ij, nfuncs, cutoff, lmbda):
        d_ij_ch_domain = 1. - 2. * ((torch.exp(-lmbda * (d_ij / cutoff - 1.)) - 1.) / (torch.exp(lmbda) - 1.))
        chebpol = radial_functions.chebvander(d_ij_ch_domain, nfuncs)
        chebpol = chebpol[:, 0, :]
        cos_cut = radial_functions.cutoff_func_cos(d_ij, cutoff)
        gk = torch.where(torch.equal(chebpol, 1.),
                      chebpol * cos_cut,
                      0.5 * (1 - chebpol) * cos_cut, name='where_chebexp')

        # y00 = 1  # / tf.sqrt(4 * torch.Tensor(np.pi, dtype=var_dtype))
        if nfuncs == 1:
            return gk[:, :-1]
        else:
            return gk  # * y00
    ...
    def cheb_pow(self, d_ij, nfuncs, cutoff, lmbda):
        d_ij_ch_domain = 2.0 * (1.0 - torch.abs(1.0 - d_ij / cutoff) ** lmbda) - 1.0

        chebpol = radial_functions.chebvander(d_ij_ch_domain, nfuncs + 1)
        chebpol = chebpol[:, 0, :]
        chebpol = 0.5 - 0.5 * chebpol[:, 1:]
        res = torch.where(torch.lt(d_ij, cutoff), chebpol, torch.zeros_like(chebpol, dtype=torch.float64), name='where_chebpow')

        return res

    def bessel_s(self, d_ij, nfuncs, cutoff):
        y = radial_functions.simplified_bessel(d_ij, cutoff, nfuncs)
        res = torch.where(torch.lt(d_ij, cutoff), y, torch.zeros_like(y, dtype=torch.float64), name='where_bessels')

        return res

    def legendre(self, d_ij, nfuncs, cutoff, lmbda):
        d_ij_l_domain = 2.0 * (1.0 - torch.abs(1.0 - d_ij / cutoff) ** lmbda) - 1.0

        legpol = radial_functions.legendre(d_ij_l_domain, nfuncs + 1)
        legpol = (legpol[:, 1:] - 1) / 2
        res = torch.where(torch.lt(d_ij, cutoff), legpol, torch.zeros_like(legpol, dtype=torch.float64), name='where_legpol')
    ...
    def flat_gather_nd(self, params, indices):
        ...
        params_flat = params.view(torch.cat([[-1], gather_shape], axis=0))
        axis_step = torch.cumprod(params_shape[:idx_dims], exclusive=True, reverse=True)
        indices_flat = torch.sum(indices * axis_step, axis=-1)
        result_flat = params_flat[indices_flat]

        return result_flat.view(torch.cat([idx_shape[:-1], gather_shape], axis=0)))

    def compute_core_repulsion(self, d_ij, core_lmbda, core_pre, core_cut, core_dcut, sum_ind):
        phi_core = core_pre * torch.exp(-core_lmbda * d_ij ** 2) / d_ij
        ...
        phi_core.scatter_reduce_(
            dim=0,
            index=self.index.bond_sum_ind[sum_ind])
            src=phi_core * (radial_functions.cutoff_func_poly(d_ij, core_cut, core_dcut))
            reduce='sum'
        )
    ...
    def f_cut_core_rep(self, rho_core, rho_core_cut, drho_core_cut):
        condition1 = torch.le(rho_core_cut, rho_core)
        condition2 = torch.lt(rho_core, rho_core_cut - drho_core_cut)

        res = torch.where(condition1, torch.zeros_like(rho_core, dtype=torch.float64),
                       torch.where(condition2, torch.ones_like(rho_core, dtype=torch.float64),
    ...
    def compute_bbasis_funcs(self, a_munlm_r, a_munlm_i, cntrl_at, rank):
        a_r = torch.permute(a_munlm_r[cntrl_at], [1, 2, 3, 0])
        a_i = torch.permute(a_munlm_i[cntrl_at], [1, 2, 3, 0])
        ...
        b_base = prod_r * torch.as_tensor(self.config.central_atom[cntrl_at][rank - 2].genCG, dtype=torch.float64)
        # b_base = tf.transpose(tf.math.segment_sum(b_base, self.config.central_atom[cntrl_at][rank - 2].msum), [1, 0])
        b_base = torch.permute(b_base.scatter_reduce_(dim=0, index=self.config.central_atom[cntrl_at][rank - 2].msum, reduce='sum'), [1, 0])

        return b_base

    def integrate(self, func, x, dx, rcut):
        f = torch.sum(torch.abs(func), axis=[1, 2]).view([-1, 1])
        trapz = torch.sum(x ** 2 * f) * dx
        trapz /= rcut ** 2

        return trapz.view([-1, 1])

    def compute_smoothness_reg(self):
        raise NotImplementedError("Need to figure out how to replace tf.GradientTape properly. See example: https://stackoverflow.com/questions/64856195/what-is-tape-based-autograd-in-pytorch")

        start = torch.Tensor(1e-10, dtype=torch.float64)
        stop = torch.Tensor(self.rcut, dtype=torch.float64)
        d_cont = torch.linspace(start, stop - 1e-5, 100).view([-1, 1])
        ...
        with tf.GradientTape(persistent=True) as tape1:
            ...
            with tf.GradientTape(persistent=True) as tape2:
                ...
                for c_i, c in enumerate(self.index.bond_combs):
                    ...
                    rcuts += [torch.Tensor(bond_spec.rcut, dtype=torch.float64)]
                    g_cont = self.radial_function(d_cont,
                                                  nfunc=nradbasei,
                                                  ftype=bond_spec.radbasename,
                                                  cutoff=rcuts[c_i],
                                                  lmbda=torch.Tensor(bond_spec.radparameters[0],
                                                                    dtype=torch.float64))  # [None, nradbase]
    ...
    def eval_atomic_energy(self, r_ij):
        d_ij = torch.linalg.norm(r_ij, axis=1).view([-1, 1])
        ...
        ynlm_r = torch.unsqueeze(ylm_r, 1) * self.factor4pi  # [None, 1, (lmax+1) * (lmax+1)]
        ynlm_i = torch.unsqueeze(ylm_i, 1) * self.factor4pi  # [None, 1, (lmax+1) * (lmax+1)]

        part_d_ij = torch_dynamic_partition(d_ij, self.index.bond_partition, self.index.ncombs)
        part_ylm_r = torch_dynamic_partition(ynlm_r, self.index.bond_partition, self.index.ncombs)
        part_ylm_i = torch_dynamic_partition(ynlm_i, self.index.bond_partition, self.index.ncombs)

        radial_coefs = self.fit_coefs[:self.total_num_crad]
        basis_coefs = self.fit_coefs[self.total_num_crad:]
        a_munlm_r = torch.zeros([self.index.nat, self.nelements, self.nradmax, (self.lmax + 1) ** 2], dtype=torch.float64)
        a_munlm_i = torch.zeros([self.index.nat, self.nelements, self.nradmax, (self.lmax + 1) ** 2], dtype=torch.float64)
        rho_r1, phi_core, f_cut_core = [], [], []
        dsor = torch.Tensor(0, dtype=torch.float64)
        for c_i, c in enumerate(self.index.bond_combs):
            ...
            nradbasei = bond_spec.nradbasemax
            crad = radial_coefs[slice(*self.bond_to_slice[c])]
            crad = crad.view([nradmaxi, lmaxi + 1, nradbasei])

            imat = torch.eye(nradmaxi, batch_shape=[lmaxi + 1], dtype=torch.float64)
            wwt = torch.matmul(torch.permute(crad, [1, 0, 2]), torch.permute(crad, [1, 2, 0]))
            if self.rankmax > 1:
                dsor += torch.mean((wwt - imat) ** 2)

            imat = torch.eye(nradbasei, batch_shape=[lmaxi + 1], dtype=torch.float64)
            wtw = torch.matmul(torch.permute(crad, [1, 2, 0]), torch.permute(crad, [1, 0, 2]))
            if self.rankmax > 1:
                dsor += torch.mean((wtw - imat) ** 2)

            phi_ij = self.compute_core_repulsion(part_d_ij[c_i],
                                                 core_lmbda=torch.Tensor(bond_spec.lambdahc, dtype=torch.float64),
                                                 core_pre=torch.Tensor(bond_spec.prehc, dtype=torch.float64),
                                                 core_cut=torch.Tensor(bond_spec.rcut_in, dtype=torch.float64),
                                                 core_dcut=torch.Tensor(bond_spec.dcut_in, dtype=torch.float64),
                                                 sum_ind=c_i)
            f_cut_core_ij = self.f_cut_core_rep(phi_ij,
                                                rho_core_cut=torch.Tensor(self.embed_spec[c[0]].rho_core_cutoff,
                                                                         dtype=torch.float64),
                                                drho_core_cut=torch.Tensor(self.embed_spec[c[0]].drho_core_cutoff,
                                                                          dtype=torch.float64))
            phi_core += [phi_ij]
            ...
            comb_gjk = self.radial_function(part_d_ij[c_i],
                                            nfunc=nradbasei,
                                            ftype=bond_spec.radbasename,
                                            cutoff=torch.Tensor(bond_spec.rcut, dtype=torch.float64),
                                            lmbda=torch.Tensor(bond_spec.radparameters[0],
                                                              dtype=torch.float64))  # [None, nradbase]
            comb_gjk = comb_gjk * (1 - radial_functions.cutoff_func_poly(part_d_ij[c_i],
                                                                    torch.Tensor(bond_spec.rcut_in, dtype=torch.float64),
                                                                    torch.Tensor(bond_spec.dcut_in, dtype=torch.float64)))
            # gk_i = tf.math.segment_sum(comb_gjk, self.index.bond_sum_ind[c_i])
            gk_i = torch.zeros_like(comb_gjk).scatter_reduce_(dim=0, index=self.index.bond_sum_ind[c_i], reduce='sum')

            coefs_r1 = torch.gather(basis_coefs, self.coefs_r1_part[c], dim=0).view([-1, ndensity])
            e = torch.matmul(gk_i, coefs_r1)
            rho_r1 += [torch.nn.functional.pad(e, [[0, 0], [0, self.ndensity - ndensity]])]

            rj_nl = torch.einsum('jk,nlk->jnl', comb_gjk, crad)
            rj_nl = torch.nn.functional.pad(rj_nl, [[0, 0], [0, self.nradmax - nradmaxi], [0, self.lmax - lmaxi]])
            rj_nlm = torch.gather(rj_nl, sh.l_tile, axis=2)
            ...
            a_nlm_r = torch.zeros_like(aj_nlm_r).scatter_reduce_(dim=0, index=self.index.bond_sum_ind[c_i])
            a_nlm_i = torch.zeros_like(aj_nlm_i).scatter_reduce_(dim=0, index=self.index.bond_sum_ind[c_i])

            update_index = torch.zeros_like(self.index.atomic_stitching[c_i]) + torch.Tensor(c[1], dtype=tf.int32)
            update_index = torch.cat([self.index.atomic_stitching[c_i], update_index], axis=1)
            a_munlm_r = a_munlm_r.index_add_(update_index, a_nlm_r)
            a_munlm_i = a_munlm_i.index_add_(update_index, a_nlm_i)

        if self.compute_orthogonality:
            self.aux += [dsor.view([-1, 1])]

        rho_r1 = tf.math.unsorted_segment_sum(torch.cat(rho_r1, 0),
                                              tf.reshape(torch.cat(self.index.atomic_stitching, 0), [-1]),
                                              num_segments=self.index.nat)
        rho_r1 = torch.zeros_like(torch.cat([rho_r1, tensor.zeros(1)]).scatter_reduce_(dim=0, index=self.index.bond_sum_ind[c_i])

        phi_core = tf.math.unsorted_segment_sum(torch.cat(phi_core, 0),
                                                tf.reshape(torch.cat(self.index.atomic_stitching, 0), [-1]),
                                                num_segments=self.index.nat)
        inner_cut = tf.math.unsorted_segment_mean(torch.cat(f_cut_core, 0),
                                                  tf.reshape(torch.cat(self.index.atomic_stitching, 0), [-1]),
                                                  num_segments=self.index.nat)
        ...
        if self.rankmax > 1:
            ...
            basis = [torch.cat(cent_at, axis=1) for cent_at in collect_basis]
            ...
            for c_i in range(self.nelements):
                ...
                for dens in range(ndensity):
                    en_sum += torch.Tensor(fs_parameters[2 * dens], dtype=torch.float64) \
                              * self.embedding_function(safe_rho[:, dens],
                                                        torch.Tensor(fs_parameters[2 * dens + 1], dtype=torch.float64),
    ...
    def f_exp_old(self, rho, mexp):
        return tf.where(tf.less(tf.abs(rho), torch.Tensor(1e-10, dtype=torch.float64)), mexp * rho,
                        self.en_func_old(rho, mexp))

    def en_func_old(self, rho, mexp):
        w = torch.Tensor(10., dtype=torch.float64)
        y1 = w * rho ** 2
        g = tf.where(tf.less(torch.Tensor(30., dtype=torch.float64), y1), 0. * rho, tf.exp(tf.negative(y1)))
    ...
    def f_exp_shsc(self, rho, mexp):
        ...
        cond = tf.abs(tf.ones_like(rho, dtype=torch.float64) * mexp - torch.Tensor(1., dtype=torch.float64))
...
class IndexPartitioner(object):
    ...
    def compute_bond_partitioning(self):
        ...
        for i, comb in enumerate(self.bond_combs):
            cond_el1 = tf.equal(self.mu_i, torch.Tensor(comb[0], dtype=tf.int32))
            cond_el2 = tf.equal(self.mu_j, torch.Tensor(comb[1], dtype=tf.int32))
